...
Ray raydirection(glm::ivec2 resolution, glm::ivec2 pixel, Camera& cam) {
	...
	glm::vec3 camSpaceRayDirection{
		normalizedPixelPos.x * cam.halfscreen,
		normalizedPixelPos.y * cam.halfscreen * cam.aspect,
		-1
	};
	camSpaceRayDirection = glm::normalize(camSpaceRayDirection);
	glm::vec3 worldSpaceRayDirection = glm::normalize(cam.invVMatrix * glm::vec4(camSpaceRayDirection, 0.0f));
...
void displayTex(GLFWwindow* window, GLuint shader, GLuint rsttex) {
...
void makeRST(RaySpaceTree* rst, std::vector<Vertex> &vertices, GLuint vao, int no_triangles, GLuint rstshader, int rstwidth, int rstheight, int option) {
...
int main() {
	...
	makeRST(&rst, vertices, vao, noPrimitives, rstProgram, w, h, 0);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//glm::mat4 mvp = mainCamera.vmMatrix(width, height);
