...
void RSTBuilderExact::filterSplittingLines(RaySpaceTree* rst, Node* leaf, std::vector<Ray>& splitlines, std::vector<bool>& sides,
											...
											bool storeCombi, std::vector<Ray>& validCombis) {
	...
	for (int i = 0; i < splitCombi4.size(); i++) {
		...
		for (Line4& r : intersectLines) {
			bool inbox;
			if (checkRayAndReverse(rst, NULL, r, leaf, false, inbox, lines4, false)) {
...
void RSTBuilderExact::getSplitLinesInLeaf(RaySpaceTree* rst, Node* leaf, std::vector<Ray>& splitLinesf, 
											std::vector<bool>& sideLinesf, bool storeCombi, std::vector<Ray>& validCombis) {
	std::vector<Ray> splitLines;
	std::vector<bool> sideLines;

	rst->getSplittingLinesInNodeWithSide(leaf, splitLines, sideLines);
	if (!rst->alldir) {
	...
	filterSplittingLines(rst, leaf, splitLines, sideLines, splitLinesf, sideLinesf, storeCombi, validCombis);
}
